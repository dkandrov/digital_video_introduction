...
# Интро
Это краткое введение в видеотехнологии. Хотя оно и предназначено для разработчиков программного обеспечения и инженеров, мы хотим, чтобы его мог освоить **любой желающий**. Эта идея родилась во время [мини-семинара для новичков в области видео технологий](https://docs.google.com/presentation/d/17Z31kEkl_NGJ0M66reqr9_uTG6tI5EDDVXpdPKVuIrs/edit#slide=id.p).
...
Все **практические упражнения выполняются из клонированной папки**. Для **примеров jupyter** вы должны запустить сервер `./s/start_jupyter.sh`, и посетить URL в браузере, который будет указан в консольном выводе.
...
**Изображение** можно представлять в форме **2D матрицы**. Если мы думаем о **цветах**, то можно экстраполировать это представление в **3D матрицу**, где **добавочные измерения** показывают **цветную информацию**.

Если мы решили представить эти цвета с помощью [основных цветов (красный, зеленый и синий)](https://en.wikipedia.org/wiki/Primary_color), мы определим три плоскости: первая - для **красного**, вторая - для **зеленого**, и последняя - для **синего** цвета.

![изображение это 3D матрица](/i/image_3d_matrix_rgb.png "Изображение это 3D матрица")

Мы назовем каждую точку на этой матрицы **пикселем**[^1] (элемент изображения). Один пиксел отражает **интенсивность** (обычно числовое значение) данного цвета. Например, **красный пиксель** имеет 0 зеленого, 0 синего, и максимально красного. **Розовый пиксель** состоит из комбинации трех цветов - **Красный=255, Зеленый=192, Синиий=203**.

[^1]: pixel на английском

> #### Другие способы кодирование изображений
> Для представления цветов, составляющих изображение, можно использовать множество других моделей. Например, мы можем использовать индексированную палитру, в которой для представления каждого пикселя потребуется всего один байт вместо трех, необходимых при использовании модели RGB. В такой модели для представления цвета можно использовать не 3D-матрицу, а 2D-матрицу, что позволит сэкономить память, но даст меньше вариантов цветов.

>
> ![сбор цветов приставки NES](/i/nes-color-palette.png "сбор цветов приставки NES")

Например, посмотрите на рисунок ниже. Первая лицо - полностью цветное. Остальные представлены красным, зеленым и синим планами (отображаются в виде оттенков серого).

![Интенсивность RGB каналов](/i/rgb_channels_intensity.png "Интенсивность RGB каналов")

Мы видим, что **красный цвет** вносит больший вклад** (самые яркие части на втором лице) в конечный цвет, в то время как вклад **синего цвета** в основном **заметен только в глазах Марио** (последнее лицо) и части его одежды, посмотрите, как **все плоскости вносят меньший вклад** (самые темные части) в **усы Марио**.

У каждого канала определённое количество битов, так называема **битовая глубина**[^2]. Если у нас **8 битов** (между 0 и 255и) для каждого цвета, то битовая глубина у изображений RGB (8 бит * 3 цвета) в размере **24 бита**, то есть 2^24 разных цвета.

[^2]: bit depth

> **Хорошо знать** [как изображения передаётся из настоящего мира в биты](http://www.cambridgeincolour.com/tutorials/camera-sensors.htm).

Другая характеристика изображений это **разрешение**[^3], то есть количество пикселей в одном измерении. Обычно оно показывается в формате ширина х высота, как на **4х4** картинке снизу.

[^3]: resolution

![Разрешение изображения](/i/resolution.png "Разрешение изображения")

> #### Практика: экспериментируем цветами и изображениями
> Можно [играться с цветами в картинках](/image_as_3d_array.ipynb) используя [jupyter](#how-to-use-jupyter) (python, numpy, matplotlib и т.д).
>
> Так же можно понять [как фильтры, (резкость, размытость...) работают](/filters_are_easy.ipynb).

Еще качество с которым мы встречаемся при работе с видео и изображениями это **соотношение сторон (AR)**[^4] которое просто описывает пропорции ширины сравнительно с высотой изображения или пикселя.

[^4]: aspect ratio

Когда люди говорят что изображение или фильм **16х9**, они называют цифры **Соотношение сторон дисплея (DAR)**[^5]. Так же и бывает **Соотношение сторон пикселя(PAR)**[^6].
...
И конечно, "видео" мы определяем как **последовательность *n*-кадров** во **времени**, что можно обозначить как дополнительное измерение поверх размера и цвета, где *n* это количество кадров в секунду (FPS[^5]).

[^5]: Frames per second

![видео](/i/video.png "видео")

**Битрейт**[^6] это сколько нужно битов что бы показать секунду видео.

[^6]: bit rate

> битрейт = ширина * высота * битовая глубина * кадров в секунду

Например, видео которое 30 FPS, 24 бита на пиксел, с разрешением 480х240 использует **82,944,000 битов в секунду** или 82 мегабита/с (Mbps) (30*480х240х24) без компрессии/снижения качества.

Когда **бит рейт** постоянный то его называют постоянным битрейтом (**CBR**[^7]), когда он меняется со временем тогда это переменный битрейт (**VBR**[^8]).
...
В ранние годы инженеры придумали технику, позволяющую удвоить воспринимаемую частоту кадров видеодисплея **без дополнительного потребления полосы пропускания**. Эта техника известна как **чересстрочное развёртка**[^9]; по сути, она передает половину экрана в одном "кадре", а другую половину – в следующем "кадре".

[^9]: interlaced video [Чересстрочная развёртка](https://ru.wikipedia.org/wiki/%D0%A7%D0%B5%D1%80%D0%B5%D1%81%D1%81%D1%82%D1%80%D0%BE%D1%87%D0%BD%D0%B0%D1%8F_%D1%80%D0%B0%D0%B7%D0%B2%D1%91%D1%80%D1%82%D0%BA%D0%B0)

Сегодня экраны в основном отображают изображения с использованием **техники прогрессивной развёртки**[^10]. Прогрессивная развёртка – это способ отображения, хранения или передачи движущихся изображений, при котором все строки каждого кадра рисуются последовательно.

[^10]: progressive scan technique [Прогрессивная развёртка](https://ru.wikipedia.org/wiki/%D0%9F%D1%80%D0%BE%D0%B3%D1%80%D0%B5%D1%81%D1%81%D0%B8%D0%B2%D0%BD%D0%B0%D1%8F_%D1%80%D0%B0%D0%B7%D0%B2%D1%91%D1%80%D1%82%D0%BA%D0%B0)

![сравнение чересстрочной и прогрессивной развёртки](/i/interlaced_vs_progressive.png "сравнение чересстрочной и прогрессивной развёртки")

Теперь у нас есть представление того как xранится **изображение** в цифровом формате, как его **цвета** расположены, сколько **битов в секунду** мы используем что бы показывать видео, если это постоянный битрейт (CBR) или переменный битрейт (VBR), и как это связано с **разрешением** при данной **частоте кадра**. Так же мы более знакомы с терминами наподобие **чересстрочная развёртка**, PAR, и т.д.

> #### Практика: Проверка свойств видео
> Вы можете [проверить оговоренные свойства с помощью ffmpeg или mediainfo.](https://github.com/leandromoreira/introduction_video_technology/blob/master/encoding_pratical_examples.md#inspect-stream)
...
> <sup>*</sup> Эту цифру мы получаем умножая 1280 х 720 х 24 х 30 х 3600 (ширина, высота, битовая глубина, чистота кадра и время в секундах).
